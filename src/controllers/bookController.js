...
const getBooksInYear= async function(req, res){
    ...
    let allBooksInYear = await BookModel.find({year:inputYear}).select({bookName: 1, authorName: 1, _id:0, year: 1})
...
const getRandomBooks = async function (req, res) {
    let allBooks= await BookModel.find({$or:[{stockAvailable: true}, {totalPages:{$gt:500}}]}).select({ bookName: 1, authorName: 1, totalPages: 1, stockAvailable:1, _id: 0})
    res.send({msg: allBooks})
}

module.exports= {createBook, getBooksData, getBooksInYear, getParticularBooks, getXINRBooks, getRandomBooks}
